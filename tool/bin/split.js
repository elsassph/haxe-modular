...
Bundler.prototype = {
	generate: function(src,output,commonjs,debugSourceMap) {
		...
		while(_g11 < _g2) {
			...
			haxe_Log.trace("Emit " + bundleOutput,{ fileName : "Bundler.hx", lineNumber : 95, className : "Bundler", methodName : "generate"});
	...
	,emitDebugMap: function(src,bundle,rawMap) {
		...
		try {
			...
			haxe_Log.trace("[WARNING] error while generating debug map for " + bundle.name + ": " + Std.string(err),{ fileName : "Bundler.hx", lineNumber : 231, className : "Bundler", methodName : "emitDebugMap"});
			return null;
		}
	}
...
Extractor.prototype = {
	process: function(mainModule,modulesList,debugMode) {
		...
		haxe_Log.trace("Bundling...",{ fileName : "Extractor.hx", lineNumber : 48, className : "Extractor", methodName : "process"});
		this.moduleMap = { };
		this.parenting = new graphlib_Graph();
		this.moduleTest = { };
		if(this.parser.typesCount == 0) {
			haxe_Log.trace("Warning: unable to process (no type metadata)",{ fileName : "Extractor.hx", lineNumber : 54, className : "Extractor", methodName : "process"});
...
HxSplit.dumpModules = function(output,extractor) {
	haxe_Log.trace("Dump bundles: " + output + ".json",{ fileName : "HxSplit.hx", lineNumber : 53, className : "HxSplit", methodName : "dumpModules"});
...
HxSplit.dumpGraph = function(output,g) {
	haxe_Log.trace("Dump graph: " + output + ".graph",{ fileName : "HxSplit.hx", lineNumber : 66, className : "HxSplit", methodName : "dumpGraph"});
	var out = "";
	var _g = 0;
	var _g1 = g.nodes();
	while(_g < _g1.length) {
...
var Parser = function(src,withLocation,commonjs) {
	...
	var t1 = new Date().getTime();
	haxe_Log.trace("Parsed in: " + (t1 - t0) + "ms",{ fileName : "Parser.hx", lineNumber : 31, className : "Parser", methodName : "new"});
	this.buildGraph(commonjs);
	var t2 = new Date().getTime();
	haxe_Log.trace("AST processed in: " + (t2 - t1) + "ms",{ fileName : "Parser.hx", lineNumber : 35, className : "Parser", methodName : "new"});
};
...
Parser.prototype = {
	...
	,buildGraph: function(commonjs) {
		...
		haxe_Log.trace("Stats: " + cpt + " types, " + refs + " references",{ fileName : "Parser.hx", lineNumber : 66, className : "Parser", methodName : "buildGraph"});
		this.typesCount = cpt;
		this.graph = g;
	}
	...
	,walkDeclarations: function(body) {
		...
		while(_g < body.length) {
			...
			case "VariableDeclaration":
				...
				break;
			default:
				haxe_Log.trace("WARNING: Unexpected " + node.type + ", at character " + node.start,{ fileName : "Parser.hx", lineNumber : 158, className : "Parser", methodName : "walkDeclarations"});
			}
		}
	}
...
Reporter.prototype = {
	save: function(output) {
		...
		haxe_Log.trace("Size report: " + output + ".stats.json",{ fileName : "Reporter.hx", lineNumber : 30, className : "Reporter", methodName : "save"});
...
SourceMap.prototype = {
	emitMappings: function(nodes,offset) {
		...
		try {
			...
			haxe_Log.trace("Invalid source-map",{ fileName : "SourceMap.hx", lineNumber : 119, className : "SourceMap", methodName : "emitMappings"});
			return null;
		}
	}
