...
Bundler.prototype = {
	...
	,process: function(modules,debugMode) {
		...
		if(debugMode) {
			...
			while(key.hasNext()) mainNodes.push(key.next());
		}
		var _g1 = [];
		var _g11 = 0;
	...
	,deduplicate: function(bundles,mainNodes,debugMode) {
		...
		while(_g3 < dupes.length) {
			var node2 = dupes[_g3];
			++_g3;
			if(g.predecessors(node2).filter(function(preNode) {
...
Main.main = function() {
	...
	var debugMode = HxOverrides.remove(args,"-debug");
	if(args.length < 3) {
...
Parser.prototype = {
	...
	,inspectExpression: function(expression,def) {
		...
		case "CallExpression":
			...
			if(this.getIdentifier(expression.callee.property).length > 0 && name1.length > 0 && this.types.exists(name1[0])) {
...
var SourceMap = function(input,src) {
	...
	this.source = new sourcemap_SourceMapConsumer(JSON.parse(js_node_Fs.readFileSync(this.fileName).toString()));
};
